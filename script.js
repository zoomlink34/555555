...
viewport.addEventListener('wheel', (e) => {
    e.preventDefault();
    scale = Math.min(Math.max(scale * (e.deltaY > 0 ? 0.9 : 1.1), 0.03), 10);
    updateUI();
}, { passive: false });

// মাউস ড্র্যাগ
viewport.onmousedown = (e) => { this.isDown = true; this.sX = e.clientX - pX; this.sY = e.clientY - pY; viewport.style.cursor = 'grabbing'; };
window.onmouseup = () => { this.isDown = false; viewport.style.cursor = 'grab'; };
...
function searchPlot() {
    ...
    if (id < 1 || id > 20000) return alert("Error: ID 1 to 20000");
